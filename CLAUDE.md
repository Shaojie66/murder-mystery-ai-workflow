...
`murder_wizard_web` — murder-wizard 的 Web 版本（React + FastAPI），包含落地页、项目仪表盘、信息矩阵编辑器等。
...
| 剧本创作 | Claude / GPT-4o / Ollama / MiniMax | 大纲扩展、逐章扩写、文风润色 |
...
│   ├── llm/              # LLM 适配层
│   │   ├── client.py     # Claude + OpenAI + Ollama + MiniMax 适配器
...
├── murder_wizard_web/     # Web 版（React + FastAPI）
│   ├── backend/
│   │   ├── main.py       # FastAPI 入口
│   │   ├── api/          # API 端点
│   │   │   ├── projects.py   # 项目 CRUD
...
│   │   │   ├── costs.py       # 消耗统计 API
│   │   │   ├── settings.py    # 设置 API
│   │   │   └── landing.py      # 落地页邮箱订阅 API
│   │   └── core/
│   │       ├── phase_runner_web.py  # SSE 包装器
...
│           ├── pages/            # 页面组件
│           │   ├── Dashboard.tsx      # 项目列表
│           │   ├── ProjectView.tsx    # 项目详情
│           │   ├── PhaseExecution.tsx # 阶段执行（SSE）
│           │   ├── MatrixEditor.tsx   # 信息矩阵编辑器
│           │   ├── FileEditor.tsx     # Monaco 编辑器
│           │   ├── AuditView.tsx     # 审计报告
│           │   ├── CostPage.tsx      # 消耗统计
│           │   ├── Settings.tsx      # LLM/Notion/Obsidian 配置
│           │   └── Subscription.tsx  # 订阅管理
│           ├── landing/          # 落地页
│           │   ├── LandingA.tsx      # 版本A（技术向）
│           │   ├── LandingB.tsx      # 版本B（新手友好）
│           │   ├── LandingPage.tsx    # A/B 路由分发
│           │   └── components/
│           │       ├── Modal.tsx           # 预约弹窗
│           │       └── OnboardingModal.tsx # 注册引导弹窗
│           ├── hooks/
│           │   └── useAnalytics.ts   # 事件追踪 Hook
│           └── components/layout/
│               ├── Layout.tsx   # 页面布局
│               └── Sidebar.tsx  # 侧边栏导航
...
| `ANTHROPIC_API_KEY` | - | Claude API（provider=claude） |
| `OPENAI_API_KEY` | - | GPT-4o（provider=openai） |
| `MINIMAX_API_KEY` | - | MiniMax（provider=minimax） |
...
|------|------|
| `/` | Dashboard（项目列表） |
| `/settings` | 全局设置（LLM / Notion / Obsidian） |
| `/subscription` | 订阅管理（Pro 版本） |
| `/projects/:name` | 项目详情 |
| `/projects/:name/phase/:n` | 阶段执行（SSE 流式） |
| `/projects/:name/matrix` | 信息矩阵编辑器 |
| `/projects/:name/files/:filename` | 文件编辑器 |
| `/projects/:name/audit` | 穿帮审计报告 |
| `/projects/:name/costs` | API 消耗统计 |
| `/landing` | 落地页（A/B 随机） |
| `/landing-a` | 落地页A（技术向 CLI 用户） |
| `/landing-b` | 落地页B（新手友好引导式） |
...
6. Web 端通过 `/api/landing/subscribe` 收集落地页邮箱，存储在内存（生产环境用数据库）

## 测试

```bash
pytest tests/ -v
# 88 tests: state_machine, session, llm_client, llm_cache, rate_limit, truth_files, api_integration
```
...
### Aesthetic Direction
- **Visual medium**: Terminal UI (Rich library) + Web UI (React + Tailwind) — 非 GUI，无 CSS/网页
...
```
--bg-base: #12110F        (主背景)
--bg-raised: #1C1917      (卡片背景)
--border: #3D3532          (边框)
--text-cream: #F5F0E8     (主文字)
--text-muted: #8A8078    (次要文字)
--text-faint: #6B6460      (辅助文字)
--accent-crimson: #9B1C1C (主强调色)
--accent-gold: #C77A08    (次强调色)
```
